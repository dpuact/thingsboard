...
import { HistoryWindowType, Timewindow, calculateIntervalStartEndTime } from '@shared/models/time/time.models';
import { Observable, Subscription, forkJoin, fromEvent, merge, of } from 'rxjs';
...
export class EntitiesTableComponent extends PageComponent implements IEntitiesTableComponent, AfterViewInit, OnInit, OnChanges {
  ...
  openJsonConfigurationDialog() {
    const dialogRef = this.dialog.open(JsonConfigurationDialogComponent, {
      width: '1200px',
      height: '850px',
  ...
  openReportDialog(reportType: string) {
  ...
  reportEnabled() {
    return this.entitiesTableConfig.reportEnabled;
  }
  ...
  resetSortAndFilter(update: boolean = true, preserveTimewindow: boolean = false) {
    ...
    if (sortable) {
      console.log('SortableId:', sortable.id);
      this.sort.active = sortable.id;
    } else {
      console.error('Sortable is undefined or null.');
    }
