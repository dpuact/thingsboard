...
import { ReportDialogComponent } from '../../components/report/report-dialog.component';
...
export class CustomersTableConfigResolver implements Resolve<EntityTableConfig<Customer>> {
  ...
  openReportDialog($event: Event, reportType: string, customer: Customer) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  onCustomerAction(action: EntityAction<Customer>, config: EntityTableConfig<Customer>): boolean {
    switch (action.action) {
      case 'open':
        ...
        return true;
      case 'openReportDialog':
        this.openReportDialog(action.event, 'CustomerDetail', action.entity);
        return true;
      case 'manageUsers':
